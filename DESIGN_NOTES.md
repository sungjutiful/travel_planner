...
필수 요구사항이 아니라 안정성/예외 처리에 먼저 집중함.

- 복수 지역 추천: recommended_city → recommended_cities 배열로 변경, 지역별 반복 검색
- 결과 캐싱: 실행 전 같은 날짜의 raw_data.json 존재 여부 확인 → 있으면 API 호출 생략

다른 API로 교체 시: LLM은 call_openai(), 지도 API는 search_restaurants()만 교체하면 나머지 파이프라인 재사용 가능.

---

## 10. 이 미션에서 배운 핵심

여러 API의 출력을 다음 API 입력으로 연결하는 파이프라인 설계 경험. 특히 LLM처럼 자유 형식 출력을 JSON으로 강제 구조화해서, 기계적으로 처리 가능한 형태로 다음 단계에 넘기는 부분이 핵심.

날씨/행사 정보의 정확도는 평가 기준이 아님(미션 문서 명시) — 구조화된 출력과 단계 간 연결이 핵심 목표.
